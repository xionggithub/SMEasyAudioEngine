...
// AEAudioUnitInputModule 和 AEIOAudioUnit 在iOs中是绑定在一起的
- (instancetype)initWithRenderer:(AERenderer *)renderer audioUnit:(AEIOAudioUnit *)audioUnit {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:AEIOAudioUnitDidUpdateStreamFormatNotification
                                                      object:self.ioUnit
...
- (void)setPreferredNumOfInputChannels:(int)preferredNumOfInputChannels {
    _preferredNumOfInputChannels = preferredNumOfInputChannels;
    
    // 传递配置
    _ioUnit.numberOfInputChannels = preferredNumOfInputChannels;
}
